...
outputs = pd.read_csv(ICU_DIR + "outputevents.csv")
...
outputs = pd.merge(outputs,item_id,on="itemid")
...
pat_for_item = outputs.groupby("label")["subject_id"].nunique()
...
outputs = outputs.loc[outputs["label"].isin(list(frequent_labels.index))].copy()
...
outputs.to_csv("processed_data/outputevents_processed.csv", index=False)